...
@doc Markdown.doc"""
    thiele(R::QQPolyRing, bb::Function; <keyword arguments>)

Compute the univariate rational function corresponding to the black box function `bb` using Thiele's interpolation formula.

# Arguments
- `R::QQPolyRing`: the univariate polynomial ring over the rationals.
- `bb::Function`: a black box function that takes a rational number as input and returns a rational number as output.
- `retry::Int=10`: the maximum number of consecutive failures allowed when evaluating the black box function.
- `show_progress::Bool=false`: whether to show a progress bar while collecting points.
- `desc::String="Univariate rational interpolation"`: the description to show in the progress bar.
- `offset::Int=0`: the number of lines to offset the progress bar.
...
julia> R, _ = polynomial_ring(QQ, "a");

julia> f = t -> (t + 1)^2 / (t - 1)^2;
...
function thiele(
    ...
    desc::String="Univariate rational interpolation",
    offset::Int=0
    ...
    prog = ProgressBar(desc=desc, offset=offset, enabled=show_progress)
    update!(prog, 0)
    n = 1
    xᵢ = QQ(n)
